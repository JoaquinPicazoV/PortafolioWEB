...
function PROJECTS() {
  return (
    <div className="w-full flex justify-center items-center py-12 md:py-20 bg-transparent relative">
      ...
      <div className="w-full md:w-5/6 z-10">
        <div className="flex flex-col justify-center items-center mb-8 md:mb-12 border-b border-white/10 pb-4 px-2">
          <h1 className="text-lg sm:text-2xl md:text-4xl font-bold text-white font-mono uppercase tracking-widest cyber-glitch-hover cursor-default w-fit mx-auto flex flex-row flex-wrap items-center justify-center gap-2 md:gap-3 max-w-full"><span className="text-emerald-500 shrink-0">{'>'}</span> <span className="text-center">[ DESARROLLO Y SOFTWARE ]</span></h1>
          ...
          <div className="flex gap-4 md:gap-6 justify-start px-4 md:px-0 w-max md:w-full">
            {otherProjects.map((project, index) => (
              <div className="group cyber-shine bg-black/80 backdrop-blur-md border border-white/10 border-l-4 border-l-transparent hover:border-l-emerald-500 p-5 md:p-6 hover:bg-[#050505] hover:shadow-[0_0_15px_rgba(16,185,129,0.15)] transition-all duration-300 flex flex-col justify-between w-[85vw] max-w-[320px] md:w-[320px] shrink-0 snap-center relative animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }} key={index}>
                ...
                <div>
                  <span className="text-xs px-2 py-1 font-mono font-bold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">{project.type}</span>
                  <h3 className="text-xl font-bold text-white group-hover:text-emerald-400 transition-colors mt-3">{project.title}</h3>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {project.languages.map((language, i) => (
                      <div key={i} className="flex items-center bg-white/5 px-3 py-1 text-xs font-mono text-slate-300 border border-white/10">
                        <img src={language.logo} loading="lazy" className={`h-4 sm:h-5 w-auto max-w-[60px] object-contain mr-2 drop-shadow-[0_0_1px_rgba(255,255,255,0.3)] ${language.name === 'Express.js' ? 'invert brightness-0' : ''}`} />
                        <p>{language.name}</p>
                      </div>
                    ))}
                  </div>
                ...
                {project.url && (
                  <a href={project.url} target="_blank" rel="noopener noreferrer" className="mt-6 block">
                    <button className="w-full bg-white/5 text-white px-4 py-2 border border-white/10 hover:border-emerald-500/50 hover:text-emerald-400 transition-all duration-300 font-medium flex items-center justify-center gap-2">
